function RowInsert(newid){
	...
	var sort_row = table.find('tr:last').find('td').find('input.row_sort');
	var type = table.find('tr:last').find('td:last').prev('td').find('input').attr('type');
	if (sort_row.val().trim().length == 0){
		sort_row.val('0');
		var lastsort = 0;
	} else 
		var lastsort = sort_row.val();
	if (lastsort == ''){
	...
	$("<tr>").attr({id:"row"+newid})
		...
		.append('<td><input type="'+type+'" name="row_def'+(type == 'checkbox'? '[]' : '')+'"></td>')
