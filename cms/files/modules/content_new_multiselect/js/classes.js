...
function oClassConstructor(div){
	...
	this.types = [
		...
		},
		{
			...
			types : [
				...
				{name:'выпадающий мулти список (select multi)', value:'selectMulti'},
		...
		},
		{
			name : 'Прочее',
			types : [
				{name:'связка с объектом, поле "Название" (select)', value:'linkTo'},
				{name:'тип поля', value:'typeoffield'},
				{name:'пароль (sha1)', value:'password'},
				{name:'загрузка файла', value:'file'}
			]
		}
	];
	...
	this.init = function(){
		...
		this.inputs = {
			...
			newField : {
				...
				p4 : $('<input class="selectedObjectID" type="text">').css({width:80}),
				browseButton : $('<input type="button" class="browse" value="...">'),
				listDiv : $('<div>'),
	...
	this.saveField = function(){
		...
		if( this.inputs.newField.index ){
			var f = this.data.fields[this.inputs.newField.index];
				...
				f.p4 = (this.inputs.newField.p4.val() || '');
		}else{
			var newField = {
				...
				p4 : (this.inputs.newField.p4.val() || '')
			};
			var added = false;
	...
	this.buildParams = function(){
		...
		switch( this.inputs.newField.type.val() ){
			...
			case 'linkTo':
				this.inputs.newField.paramsDiv
				.append( $("<div>").append('Введите ID объекта ') )
				.append( this.inputs.newField.p4 )
				.append(
					this.inputs.newField.browseButton.click(function(){
						var tmpID = '';
						$.get(ajaxFile, {run:_RUN_, go:'getObjectsStructureAsHTML'}, function(html){
							...
							$("#treeDialog").dialog({
								...
								buttons: {
									...
									"Выбрать": function(){
										$("input.selectedObjectID").val(tmpID);
										$(this).dialog("close");
									}
								}
	...
	this.editField = function(id){
		...
		this.inputs.newField.p4.val( f.p4 );
		this.buildFields(id);
		this.buildParams();
	}
